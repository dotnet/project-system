...
public class DiffUtil
{
    ...
    {
        var lcs = (comparer is not null) ? new LCS<T>(comparer) : LCS<T>.Default;
        toString ??= new Func<T, string>(obj => obj?.ToString() ?? "");

        IList<T> expectedList = expected as IList<T> ?? [.. expected];
        IList<T> actualList = actual as IList<T> ?? [.. actual];
