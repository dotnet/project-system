...
namespace Microsoft.VisualStudio.ProjectSystem.VS.NuGet
{
    ...
    ""ProjectChanges"": {
        ...
        [Fact]
        public void ProjectRestoreInfoBuilder_WithUnversionedPackageRef_ReportsAsProjectRef()
        {
            var projectSubscriptionUpdates = GetVersionedUpdatesFromJson(@"{
    ""ProjectConfiguration"": {
        ""Name"": ""Debug|AnyCPU|netcoreapp1.0"",
    ...
    ""ProjectChanges"": {
        ...
        ""PackageReference"": {
            ...
            ""After"": {
                ""Items"": {
                    ...
                    ""..\\PackageProducingProject\\PackageProducingProject.csproj"": {
                        ""DefiningProjectDirectory"": ""C:\\Test\\Projects\\TestProj"",
                        ""DefiningProjectFullPath"": ""C:\\Test\\Projects\\TestProj\\TestProj.csproj"",
                        ""TargetFramework"": """",
                        ""RuntimeIdentifier"": """"
        ...
        ""ProjectReference"": {
            ...
            // one of the package refs is converted to a project ref
            Assert.Equal(2, tfm.ProjectReferences.Count);
            Assert.Equal(1, tfm.PackageReferences.Count);

            var definingProjectDirectory = "C:\\Test\\Projects\\TestProj";
            var definingProjectFullPath = "C:\\Test\\Projects\\TestProj\\TestProj.csproj";

            // project references
            var projectRef = tfm.ProjectReferences.Item("..\\TestLib\\TestLib.csproj");
            Assert.Equal("..\\TestLib\\TestLib.csproj", projectRef.Name);
            Assert.Equal("C:\\Test\\Projects\\TestLib\\TestLib.csproj", projectRef.Properties.Item("ProjectFileFullPath").Value);
            Assert.Equal(definingProjectDirectory, projectRef.Properties.Item("DefiningProjectDirectory").Value);
            Assert.Equal(definingProjectFullPath, projectRef.Properties.Item("DefiningProjectFullPath").Value);

            projectRef = tfm.ProjectReferences.Item("..\\PackageProducingProject\\PackageProducingProject.csproj");
            Assert.Equal("..\\PackageProducingProject\\PackageProducingProject.csproj", projectRef.Name);
            Assert.Equal("C:\\Test\\Projects\\PackageProducingProject\\PackageProducingProject.csproj", projectRef.Properties.Item("ProjectFileFullPath").Value);
            Assert.Equal(definingProjectDirectory, projectRef.Properties.Item("DefiningProjectDirectory").Value);
            Assert.Equal(definingProjectFullPath, projectRef.Properties.Item("DefiningProjectFullPath").Value);
