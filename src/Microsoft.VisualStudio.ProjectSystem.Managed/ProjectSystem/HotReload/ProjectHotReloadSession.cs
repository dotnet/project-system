...
internal sealed class ProjectHotReloadSession : IProjectHotReloadSessionInternal
{
    ...
    /// </summary>
    private static string GetStartupHookPath(string applicationTargetFramework)
    {
        var hookTargetFramework = applicationTargetFramework switch
        {
    ...
    private async ValueTask<IDeltaApplier> GetOrCreateDeltaApplierAsync(CancellationToken cancellationToken)
    {
        ...
        if (applier is null)
        {
            ...
            var suppressDeltaApplication = false;
            ...
            if (_callback is IProjectHotReloadSessionWebAssemblyCallback wasmCallback)
            {
                ...
                var targetFrameworkName = new FrameworkName(await _configuredProject.GetProjectPropertyValueAsync(ConfigurationGeneral.TargetFrameworkMonikerProperty));
                var browserRefreshServer = wasmCallback.BrowserRefreshServerProvider.Server;

                suppressDeltaApplication = wasmCallback.SuppressDeltaApplication;
            ...
            {
                client = new DefaultHotReloadClient(clientLogger, agentLogger, GetStartupHookPath(targetFramework), enableStaticAssetUpdates: true);
            }
