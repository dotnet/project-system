...
Namespace Microsoft.VisualStudio.Editors.AppDesDesignerFramework
    ...
    Friend NotInheritable Class DesignUtil
        ...
        '**************************************************************************
        Public Overloads Shared Sub ReportError(
        ...
        Public Shared Sub ShowWarning(
            ...
            DesignerMessageBox.Show(ServiceProvider, Message, GetDefaultCaption(ServiceProvider),
        ...
        '**************************************************************************
        Public Overloads Shared Function ShowMessage(
                                                      ServiceProvider As IServiceProvider,
                                                      Message As String,
                                                      Caption As String,
        ...
        Public Shared Sub DisplayTopicFromF1Keyword(ByVal ServiceProvider As IServiceProvider, ByVal keyword As String)
            If ServiceProvider Is Nothing Then Debug.Fail("NULL " & NameOf(ServiceProvider) & " - can't show help!") : Return
            If keyword Is Nothing Then Debug.Fail("NULL help " & NameOf(keyword) & " - can't show help!") : Return

            Dim vshelp As VSHelp.Help = CType(ServiceProvider.GetService(GetType(VSHelp.Help)), VSHelp.Help)

            If vshelp Is Nothing Then Debug.Fail("Failed to get " & NameOf(VisualStudio.VSHelp) & "." & NameOf(VisualStudio.VSHelp.Help) & " service from given service provider - can't show help!") : Return
            ...
            Catch ex As COMException
                ...
                Debug.Assert(Marshal.GetHRForException(ex) = &H80040305, $"Unknown COM Exception {ex} when trying to show help topic {keyword}")
            End Try
        End Sub
        ...
        Public Shared Function GetContextMenuMouseEventArgs(Control As Control, ByRef m As Message) As MouseEventArgs
            Dim x = SignedLoWord(m.LParam)
            Dim y = SignedHiWord(m.LParam)
        ...
        Public Shared Function GenerateValidLanguageIndependentIdentifier(ByVal value As String) As String
            ...
            If CodeDom.Compiler.CodeGenerator.IsValidLanguageIndependentIdentifier(value) Then Return value

            Dim chars() As Char = value.ToCharArray()
            If chars.Length = 0 Then Throw AppDesCommon.CreateArgumentException(NameOf(value))

            Dim result As New System.Text.StringBuilder(chars.Length)
            ...
            If Not CodeDom.Compiler.CodeGenerator.IsValidLanguageIndependentIdentifier(cleanIdentifier) Then
                Debug.Fail($"Failed to clean up identifier '{cleanIdentifier}'")
        ...
        Public Shared Function GetEncoding(
                                           dd As Shell.Design.Serialization.DocData
            ...
            Try
                ...
                Dim userData = TryCast(dd.Buffer, TextManager.Interop.IVsUserData)
        ...
        Public Shared Function IsClickOnceSupported(hier As IVsHierarchy) As Boolean
            If AppDesCommon.ShellUtil.IsWebProject(hier) Then Return False
            ...
            Try
                ...
                Dim cgfs2 = TryCast(cfgs(0), IVsProjectCfg2) : cgfs2.get_CfgType(GetType(IVsPublishableProjectCfg).GUID, publishableServicePtr)
                Dim publishableService = DirectCast(Marshal.GetObjectForIUnknown(publishableServicePtr), IVsPublishableProjectCfg)
                Dim publishable(1) As Integer, ready(1) As Integer
