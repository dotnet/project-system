...
Namespace Microsoft.VisualStudio.Editors.AppDesInterop
    ...
    Friend Interface IVsAppId
        ...
        ' HRESULT GetProperty([in] VSAPROPID propid, [out] VARIANT *pvar);
        <PreserveSig()>
        Function GetProperty(propid As Integer, <Out(), MarshalAs(UnmanagedType.Struct)> ByRef pvar As Object) As Integer

        ' HRESULT SetProperty([in] VSAPROPID propid, [in] VARIANT var);
        Sub SetProperty(propid As Integer, <[In](), MarshalAs(UnmanagedType.Struct)> var As Object)

        ' HRESULT GetGuidProperty([in] VSAPROPID propid, [out] GUID *pguid);
        Sub GetGuidProperty(propid As Integer, <Out()> ByRef pguid As Guid)

        ' HRESULT SetGuidProperty([in] VSAPROPID propid, [in] REFGUID rguid);
