...
Namespace Microsoft.VisualStudio.Editors.AppDesInterop
    ...
    Friend NotInheritable Class NativeMethods
        ...
        Public Shared ReadOnly VBCompilerGuid As New System.Guid(s_VB_COMPILER_GUID)
        ...
        Public Shared ReadOnly IID_IMetaDataImport As New Guid("7DAC8207-D3AE-4c75-9B67-92801A497D44")
...
#End If
            ...
            Public Sub New(
                           [source] As Object,
                           sink As Object,
                           eventInterface As Type,
                           throwException As Boolean
                          )

                Dim ex As Exception = Nothing
                If sink Is Nothing OrElse Not eventInterface.IsInstanceOfType(sink) Then
                    ex = New InvalidCastException($"The {NameOf(sink)} object does not implement the {NameOf(eventInterface)}.")
                ...
                ElseIf TypeOf [source] Is ComTypes.IConnectionPointContainer Then
                    Dim cpc = CType([source], ComTypes.IConnectionPointContainer)
                    ...
                    Else
                        ...
                        Catch e As Exception
                            _connectionPoint2 = Nothing
                            ex = New Exception($"{NameOf(IConnectionPoint)}::{NameOf(IConnectionPoint.Advise)} failed for event interface '{eventInterface.Name}'")
                        End Try
                        _cookie = CUInt(cookie2)
                    End If
                    ex = New InvalidCastException($"The source object does not expose {NameOf(IConnectionPointContainer)}.")
                End If
...
#End If
            ...
            Public Overloads Sub Disconnect(release As Boolean)
                If _cookie = 0 Then Exit Sub
                Try
                    If _connectionPoint IsNot Nothing Then

                        _connectionPoint.Unadvise(_cookie)
                        If release Then Marshal.ReleaseComObject(_connectionPoint)
                        _connectionPoint = Nothing

                    ElseIf _connectionPoint2 IsNot Nothing Then

                        _connectionPoint2.Unadvise(CInt(_cookie))
                        If release Then Marshal.ReleaseComObject(_connectionPoint2)
                        _connectionPoint2 = Nothing
                    End If
                Finally
                    _cookie = 0
...
#If DEBUG Then
                Debug.Assert(_cookie = 0, $"We should never finalize an active connection point. ({NameOf(_eventInterface)}) = {_eventInterface.FullName}), allocating code (see stack) is responsible for unhooking the ConnectionPoint by calling Disconnect.  Hookup Stack ={vbNewLine}{_callStack}")
#End If
        ...
        <PreserveSig()> Public Declare Function SetParent Lib "user32" (hwnd As IntPtr, hWndParent As IntPtr) As IntPtr
        <PreserveSig()> Public Declare Function GetParent Lib "user32" (hwnd As IntPtr) As IntPtr
        <PreserveSig()> Public Declare Function GetFocus Lib "user32" () As IntPtr
        <PreserveSig()> Public Declare Function SetFocus Lib "user32" (hwnd As IntPtr) As Integer
        <PreserveSig()> Public Declare Auto Function SendMessage Lib "user32" (hwnd As HandleRef, msg As Integer, wParam As Integer, lParam As Integer) As IntPtr
        <PreserveSig()> Public Declare Auto Function SendMessage Lib "user32" (hwnd As HandleRef, msg As Integer, wParam As Integer, lParam As TVITEM) As IntPtr
        <PreserveSig()> Public Declare Auto Function SendMessage Lib "user32" (hwnd As IntPtr, msg As Integer, wParam As IntPtr, lParam As IntPtr) As IntPtr
        <PreserveSig()> Public Declare Auto Function PostMessage Lib "user32" (hwnd As IntPtr, msg As Integer, wParam As Integer, lParam As Integer) As IntPtr
        <PreserveSig()> Public Declare Auto Function WaitMessage Lib "user32" () As Boolean
        <PreserveSig()> Public Declare Auto Function IsDialogMessage Lib "user32" (hwnd As HandleRef, ByRef msg As MSG) As Boolean
        <PreserveSig()> Public Declare Auto Function TranslateMessage Lib "user32" (ByRef msg As MSG) As Boolean
        <PreserveSig()> Public Declare Auto Function GetKeyState Lib "user32" (nVirtKey As Keys) As Int16
        ...
        ''' </summary>
        <PreserveSig()> Public Declare Auto Function GetNextDlgTabItem Lib "user32" (hDlg As IntPtr, hCtl As IntPtr, bPrevious As Boolean) As IntPtr
        <PreserveSig()> Public Declare Auto Function GetWindow Lib "user32" (Hwnd As IntPtr, uCmd As UInteger) As IntPtr
        <PreserveSig()> Public Declare Auto Function DragQueryFile Lib "shell32" (hDrop As IntPtr, iFile As Integer, lpszFile As String, cch As Integer) As Integer
        <PreserveSig()> Public Declare Function GetUserDefaultLCID Lib "kernel32" () As UInteger
        <PreserveSig()> Public Declare Function GetTopWindow Lib "user32" (Hwnd As IntPtr) As IntPtr
        <PreserveSig()> Public Declare Auto Function SetWindowLong Lib "user32" (hWnd As IntPtr, Index As Integer, Value As IntPtr) As IntPtr
        <PreserveSig()> Public Declare Auto Function GetWindowLong Lib "user32" (Hwnd As IntPtr, Index As Integer) As IntPtr
        <PreserveSig()> Public Declare Auto Function GetWindowText Lib "user32" (hWnd As IntPtr, lpString As String, nMaxCount As Integer) As Integer

        <DllImport("user32", CharSet:=CharSet.Auto)>
        Public Shared Function GetWindowRect(hwnd As IntPtr, rect As RECT) As Integer
        End Function

        Public Declare Function MoveWindow Lib "user32" (hWnd As IntPtr,
        ...
        <PreserveSig()> Public Declare Auto Function IsChild Lib "user32" (hWndParent As IntPtr, hWnd As IntPtr) As Boolean
        <PreserveSig()> Public Declare Auto Function EnableWindow Lib "user32" (hWnd As IntPtr, bEnable As Boolean) As Boolean
        '<PreserveSig()> Public Declare Auto Sub ShowWindow Lib "user32" ( Hwnd As IntPtr,  Flags As Integer)
        '<PreserveSig()> Public Declare Auto Function SetWindowPos Lib "user32" ( Hwnd As IntPtr,  HwndInsertAfter As IntPtr,
        '                                                                         x As Integer, y As Integer,
        '                                                                         cx As Integer, cy As Integer,
        '                                                                         flags As Integer) As Boolean

        <PreserveSig()> Public Declare Auto Function SystemParametersInfo Lib "user32" (uiAction As UInteger, uiParam As UInteger, pvParam As IntPtr, fWinIni As UInteger) As Integer
        <PreserveSig()> Public Declare Auto Function MsgWaitForMultipleObjects Lib "user32" (nCount As Integer, pHandles As IntPtr,
                                                                                             fWaitAll As Boolean, dwMilliSeconds As Integer, dwWakeMask As Integer) As Integer
        ...
        <DllImport("user32")> Public Shared Function GetComboBoxInfo(ByVal hwndCombo As IntPtr, ByRef info As COMBOBOXINFO) As Boolean
        End Function
